...
def collect_from_wizard() -> OnboardConfig:
    """Load config from JSON file or return defaults (no interactive wizard).
    
    Looks for config.json in:
    1. /app/data/config.json (Docker volume)
    2. ./config.json (current directory)
    3. ~/.openoutreach/config.json (home directory)
    4. Returns defaults if not found
    
    This avoids the private 'openoutreach' package dependency.
    """
    import json
    ...
    config_paths = [
        ...
        Path("./config.json"),
        Path.home() / ".openoutreach" / "config.json",
    ]
    ...
    logger.info("✓ No config.json found, using defaults. Create one to customize.")
    return OnboardConfig()
