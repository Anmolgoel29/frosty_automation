...
- `migrate_postgres_to_remote --target-url <url>` — copy all data from the current (local) Postgres into a remote Postgres such as Supabase/Neon/RDS. Source is the ambient `DATABASE_URL`; target is `--target-url` (or the `TARGET_DATABASE_URL` env var). Same dump/migrate/load shape as `migrate_sqlite_to_postgres`, but both sides are Postgres and no schema-catch-up copy is needed: it (1) `dumpdata`s the source (the parent process's default DB) with `--natural-foreign --natural-primary`, excluding `contenttypes`/`auth.permission`, (2) runs `migrate` against the target in a subprocess with the target as **default** (via a `DATABASE_URL`-overridden env, POSTGRES_* stripped so a stray fallback can't shadow it) — a subprocess for the same `Model.objects`-without-`.using()` reason, (3) `loaddata`s the export into the target, (4) prints a source-vs-target row-count comparison via a `postgres_target` connection alias built from `django_db_config(target_url)`. Refuses to run if source and target resolve to the same host/port/db. The target should be an empty/fresh DB (its own rows for loaded tables would collide on PK). Keeps the exported JSON in `data/` as a backup. Passwords are redacted in the printed source/target lines.
