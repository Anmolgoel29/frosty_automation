...
Persistent queue backed by `Task` model. Every row carries `linkedin_profile` — the account that must execute it. Worker loop in `daemon.py`: `seconds_until_active()` guard pauses outside active hours/rest days → pop oldest due task → look up that account's `AccountSession` (FAILED if it isn't running: a task is never re-homed to another account) → set campaign on session → RUNNING → dispatch via `_HANDLERS` dict → COMPLETED/FAILED. Failures captured by `failure_diagnostics()` context manager.
...
- **`daemon.py`** — `run_daemon(sessions)` worker loop over the `{profile_pk: AccountSession}` map, with active-hours guard (`ENABLE_ACTIVE_HOURS` flag, `seconds_until_active()`), `_build_qualifiers()`, `sync_sessions()`. Routes each task to `task.linkedin_profile_id`'s session. Calls `sync_sessions()` then `scheduler.reconcile(sessions)` when the queue has no ready task.
