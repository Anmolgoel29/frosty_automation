...
- `migrate_postgres_to_remote --target-url <url>` — copy all data from the current (local) Postgres into a remote Postgres such as Supabase/Neon/RDS. Source is the ambient `DATABASE_URL`; target is `--target-url` (or the `TARGET_DATABASE_URL` env var). Same dump/migrate/load shape as `migrate_sqlite_to_postgres`, but both sides are Postgres and no schema-catch-up copy is needed. It registers the target as a `postgres_target` connection alias (a fully-defaulted dict built from `django_db_config(target_url)` — Django only fills per-alias defaults like `OPTIONS`/`TIME_ZONE`/`CONN_MAX_AGE` in `configure_settings`, which is cached and never re-runs for an alias injected afterward, so the dict must carry them). Then: (1) `dumpdata`s the source (parent's default DB) with `--natural-foreign --natural-primary`, excluding `contenttypes`/`auth.permission`; (2) runs `migrate` against the target in a subprocess with the target as **default** (via a `DATABASE_URL`-overridden env, POSTGRES_* stripped so a stray fallback can't shadow it) — a subprocess because the historical data migrations query `Model.objects` without `.using()`; (3) **streams** the export into the `postgres_target` alias in-process (not the opaque `loaddata` subprocess) via a faithful port of loaddata's inner loop — `constraint_checks_disabled` during inserts, `save(using=target)` per object, deferred natural-key forward references saved at the end, `check_constraints`, then `sequence_reset_sql` so the target's PK sequences don't collide with future inserts — emitting **live timestamped progress**: a per-model row tally as each model completes and a heartbeat every ~2s (`loaded/total`, %, rows/s, ETA, current model); (4) prints a source-vs-target row-count comparison over the same alias. All phases log through a timestamped `_log()`. Refuses to run if source and target resolve to the same host/port/db. The target should be an empty/fresh DB (its own rows for loaded tables would collide on PK). Keeps the exported JSON in `data/` as a backup. Passwords are redacted in the printed source/target lines. (loaddata is per-row over the network, so this is inherently slow on a high-latency remote link — the progress output is there so a long phase 3 is legible rather than silent.)
