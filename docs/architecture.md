...
The system uses Django with a Postgres database (configured via a single `DATABASE_URL`, with `POSTGRES_*` fallbacks; see `db_url.py` and `linkedin/django_settings.py`). The key models are:
